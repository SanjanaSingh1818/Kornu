    ...
    <section className="bg-primary-50 px-6 py-24 sm:py-28">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-5">
          {COURSES.map((c, i) => {
            const translated = t.courses.cards[i] || [c.title, c.text, c.price];
            return (
            <motion.a href="#paket" key={c.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} whileHover={{ y: -6 }} viewport={{ once: true, margin: "-60px" }} transition={{ delay: i * .04, duration: .55 }}
              ...
              <div className="h-44 overflow-hidden">
                <img src={c.image} alt={translated[0]} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
              </div>
              <div className="p-5">
